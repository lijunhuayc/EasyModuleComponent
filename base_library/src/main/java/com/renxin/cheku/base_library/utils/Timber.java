...
import com.renxin.cheku.base_library.BuildConfig;
...
/**
 * Logging for lazy people. 仅仅在debug模式下才会打印日志
 */
public final class Timber {
	...
	public static abstract class Tree {
		...
		private void prepareLog(int priority, Throwable t, String message, Object... args) {
			...
			if (message == null) {
				if (t == null) {
					return; // Swallow message if it's null and there's no
							// throwable.
				}
		...
		/**
		 * Write a log message to its destination. Called for all level-specific
		 * methods by default.
		 * 
		 * @param priority
		 *            Log level. See {@link Log} for constants.
		 * @param tag
		 *            Explicit or inferred tag. May be {@code null}.
		 * @param message
		 *            Formatted log message. May be {@code null}, but then {@code t} will not be.
		 * @param t
		 *            Accompanying exceptions. May be {@code null}, but then {@code message} will not be.
		 */
		protected abstract void log(int priority, String tag, String message, Throwable t);
	}
	/**
	 * A {@link Tree Tree} for debug builds. Automatically infers the tag from
	 * the calling class.
	 */
	public static class DebugTree extends Tree {
		...
		/**
		 * Extract the tag which should be used for the message from the {@code element}. By default this will use the class name without any
		 * anonymous class suffixes (e.g., {@code Foo$1} becomes {@code Foo}).
		 * <p>
		 * Note: This will not be called if a {@linkplain #tag(String) manual
		 * tag} was specified.
		 */
		...
		final String getTag() {
			...
			// The test will pass
			// because Robolectric runs them on the JVM but on Android the
		...
		/**
		 * Break up {@code message} into maximum-length chunks (if needed) and
		 * send to either {@link Log#println(int, String, String) Log.println()} or {@link Log#wtf(String, String) Log.wtf()} for logging.
		 * 
		 * {@inheritDoc}
		 */
		@Override
		protected void log(int priority, String tag, String message, Throwable t) {
			if (!BuildConfig.DEBUG)
				return;//TODO...add by ljh 发布时过滤日志
