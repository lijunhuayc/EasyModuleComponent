package com.renxin.cheku.cooldialog.widget;
...
import com.renxin.cheku.cooldialog.R;

/**
 * Desc: 自定义样式Loading动画View
 * Created by Junhua.Li
 * Date: 2018/05/28 11:54
 */
public class RotateLoadingWidget extends View {
    ...
    public RotateLoadingWidget(Context context) {
        this(context, null);
    }

    public RotateLoadingWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RotateLoadingWidget(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainStyleAttrs(AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.RotateLoadingWidget);
        color = typedArray.getColor(R.styleable.RotateLoadingWidget_loading_color, color);
        width = (int) typedArray.getDimension(R.styleable.RotateLoadingWidget_loading_width, width);
        shadowOffset = (int) typedArray.getDimension(R.styleable.RotateLoadingWidget_shadow_offset, shadowOffset);
        isStart = typedArray.getBoolean(R.styleable.RotateLoadingWidget_loading_visible, isStart);
        typedArray.recycle();
    }
